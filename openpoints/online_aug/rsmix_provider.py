...
def pts_num_ctrl(pts_erase_idx, pts_add_idx):
    ...
    if len(pts_erase_idx) >= len(pts_add_idx):
        ...
        if num_diff == 0:
            ...
            pts_add_idx_ctrled = np.append(pts_add_idx,
...
def rsmix(data_batch, label_batch, beta=1.0, n_sample=512, KNN=False):
    ...
    rand_index = np.random.choice(data_batch.shape[0], data_batch.shape[0],
                                  replace=False)  # label dim : (16,) for model
    ...
    if KNN:
        ...
        pts_erase_idx, query_point_1 = cut_points_knn(data_batch, rand_idx_1, cut_rad, nsample=n_sample,
                                                      k=knn_para)  # B x num_points_in_radius_1 x 3(or 6)
        pts_add_idx, query_point_2 = cut_points_knn(data_batch_rand, rand_idx_2, cut_rad, nsample=n_sample,
                                                    k=knn_para)  # B x num_points_in_radius_2 x 3(or 6)
    else:
        pts_erase_idx, query_point_1 = cut_points(data_batch, rand_idx_1, cut_rad,
                                                  nsample=n_sample)  # B x num_points_in_radius_1 x 3(or 6)
        pts_add_idx, query_point_2 = cut_points(data_batch_rand, rand_idx_2, cut_rad,
                                                nsample=n_sample)  # B x num_points_in_radius_2 x 3(or 6)
    ...
    for i in range(data_batch.shape[0]):
        if pts_erase_idx[i][0][0] == data_batch.shape[1]:
            ...
            tmp_pts_replaced = np.expand_dims(np.concatenate((tmp_pts_erased, data_batch[i][dup_points_idx]), axis=0),
                                              axis=0)
            lam_tmp = 0
        else:
            pts_erase_idx_tmp = np.unique(pts_erase_idx[i].reshape(n_sample, ), axis=0)     #   [nsample, ]
            pts_add_idx_tmp = np.unique(pts_add_idx[i].reshape(n_sample, ), axis=0)         #   [nsample, ]
            pts_add_idx_ctrled_tmp = pts_num_ctrl(pts_erase_idx_tmp, pts_add_idx_tmp)       #   [nsample, ]
            tmp_pts_erased = np.delete(data_batch[i], pts_erase_idx_tmp, axis=0)  # B x N-num_rad_1 x 3(or 6)       #   [nsample, 3]
            # input("INPUT : ")
            tmp_pts_to_add = np.take(data_batch_rand[i], pts_add_idx_ctrled_tmp, axis=0)    #   [nsample,3]
            tmp_pts_to_add[:, :3] = query_dist[i] + tmp_pts_to_add[:, :3]                   #   [1, 3] + [nsample, 3]

            tmp_pts_replaced = np.expand_dims(np.vstack((tmp_pts_erased, tmp_pts_to_add)), axis=0)  #   [1, 1024, 3]
    ...
    data_batch_mixed = np.delete(pts_replaced, [0], axis=0)     #   [B, N, 3]

...
from examples.classification.train_pointwolf_utils import train_one_epoch_pointwolf, train_one_epoch_rsmix, train_one_epoch_wolfmix
...
def main(gpu, cfg, profile=False):
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if cfg.get('pointwolf', None) is not None:
            train_loss, train_macc, train_oa, _, _ = \
                train_one_epoch_pointwolf(model, train_loader,
                                optimizer, scheduler, epoch, cfg)
        elif cfg.get('rsmix_params', None) is not None:
            train_loss, train_macc, train_oa, _, _ = \
                train_one_epoch_rsmix(model, train_loader,
                                optimizer, scheduler, epoch, cfg)
        elif cfg.get('wolfmix', None) is not None:
            train_loss, train_macc, train_oa, _, _ = \
                train_one_epoch_wolfmix(model, train_loader,
                                optimizer, scheduler, epoch, cfg)
        else:
            train_loss, train_macc, train_oa, _, _ = \

...
from openpoints.dataset.modelnet_c.modelnet_c import ModelNetC, eval_corrupt_wrapper_modelnetc
...
def train_gan(cfg, gan_dict, train_loader, summary, writer, epoch, model_pointcloud):
    ...
    for i, data in tqdm(enumerate(train_loader), total=train_loader.__len__()):
        ...
        #
        # cls_loss_fakedata = cal_cls_loss(model_pointcloud=model_pointcloud, criterion_pointcloud=criterion_pointcloud, \
        #                                  data_real=input_pointcloud.permute(0, 2, 1),
        #                                  data_fake=gen_imgs.permute(0, 2, 1), \
...
def main(gpu, cfg, profile=False):
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if epoch > cfg.get('adaptpoint_adjustepoch', 0):
            ...
            if cfg.get('rsmix_params', None) is not None:
                train_loss, train_macc, train_oa, _, _ = \
                    train_one_epoch_rsmix(model, fake_train_loader,
                                    optimizer, scheduler, epoch, cfg)
            else:
                ...
                train_loss, train_macc, train_oa, _, _ = \
                    train_one_epoch(model, fake_train_loader,
                                optimizer, scheduler, epoch, cfg)
                # print(f'train time: {time.time() - train_time_t1}')
        else:
            train_loss, train_macc, train_oa, _, _ = \
                train_one_epoch(model, train_loader,
                                optimizer, scheduler, epoch, cfg)
        ...
        if (epoch+1) % 20 == 0:
            eval_corrupt_wrapper_modelnetc(model, validate_modelnetc, {'cfg': cfg}, cfg.run_dir, epoch)
        ...
        if epoch % cfg.val_freq == 0:
            ...
            # val_time_t1 = time.time()
            val_macc, val_oa, val_accs, val_cm = validate_fn(
                model, val_loader, cfg)
            # print(f'validate time: {time.time() - val_time_t1}')
    ...
    testmodelnetc(model=model, path=best_ckpt_path, cfg=cfg)
    testmodelnetc(model=model, path=last_ckpt_path, cfg=cfg)
...
@torch.no_grad()
def validate_modelnetc(split, model, cfg):
    model.eval()  # set model to eval mode
    cm = ConfusionMatrix(num_classes=cfg.num_classes)
    npoints = cfg.num_points
    data_transform = build_transforms_from_cfg('val', cfg.datatransforms_modelnet_c)
    testloader = torch.utils.data.DataLoader(ModelNetC(split=split, transform=data_transform), num_workers=int(cfg.dataloader.num_workers), \
...
def testmodelnetc(model, path, cfg):
    ckpt = torch.load(f'{path}')
    model.load_state_dict(ckpt['model'])
    epoch  = ckpt['epoch']
    eval_corrupt_wrapper_modelnetc(model, validate_modelnetc, {'cfg': cfg},
                               cfg.run_dir, epoch)